...
- Publish Point: subscribes to /clicked_point, publishes PoseStamped to /goal_pose (full 3D).
- 2D Goal Pose: RViz publishes to /goal_pose with z=0 (ground). We subscribe and when
  ...
  goal is in the drone's flight plane.
"""
...
class GoalFromRvizNode(Node):
    def __init__(self):
        ...
        self.get_logger().info(
            f"Goal from RViz: {clicked_topic} & {goal_topic} -> {goal_topic} "
    ...
    def cb_goal_pose(self, msg: PoseStamped):
        ...
        if drone_z is None:
            self.get_logger().warning(
                "2D goal received (z=%.2f) but could not get drone height from TF; publishing as-is."
                % z,
